...
### 8.1 No rate limiting on auth endpoints ✅ FIXED

> Commit: `feat(auth): add rate limiting — 10 req/min on login and register endpoints`
> `@nestjs/throttler` installed. Global guard (100 req/min) with a stricter `auth` named throttler (10 req/min) on `POST /auth/login` and `POST /auth/register`.
...
### 8.8 Both `csv-parse` and `csv-parser` are installed ✅ FIXED

> Commit: `chore(deps): remove unused csv-parser, fs, and async_hooks packages`
...
### 8.9 `fs` dummy package in dependencies ✅ FIXED
...
### 8.10 `async_hooks` as a real dependency ✅ FIXED
...
### 8.11 Weak MIME check for CSV uploads ✅ FIXED

> Commit: `fix(import): add .csv extension check alongside MIME type check`
> Both `file.mimetype.includes('csv')` and `file.originalname.endsWith('.csv')` are now checked. The actual parse stage still provides the real security gate.
...
### 8.15 Page metadata placeholder ✅ FIXED

> Commit: `fix(ui): replace placeholder page description with real app description`
